...
- 提交 / push：功能与静态发布提交 `564b6c1a`（`feat: add verified task learning landing`）已成功推送 `origin/main`；本条交接回填随后单独提交并推送。其余 `.DS_Store`、访谈 HTML、研究目录和大型未跟踪文件继续保留，未纳入提交。发布后需实际打开 Demo 与既有竞品汇报两个 GitHub Pages 地址复核。
