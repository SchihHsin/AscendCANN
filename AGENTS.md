...
- 项目汇总页改为更克制的页面级目录：按项目折叠分组，列出 31 个有明确用途的具体页面并支持搜索；不收录样式试验、过程稿和内部备份。
- 汇总页再次调整为页面级作品集：参考 CodeArts 设计材料库的“筛选 + 卡片 + 具体页面直达”信息架构，但采用深石墨、青绿和珊瑚橙的研究档案主题。新增 7 张来自目标页面真实视图的封面截图，精选 11 页以卡片展示，其余入口收在展开区。
- 页面作品集独立于原 CANN 站点导航，主身份改为“探索版昇腾社区”。页面数据改为完整卡片墙：收录社区探索、开发平台、体验运营、基准评测、研究洞察和方法工具共 40 个具体入口；筛选会隐藏非当前类别的所有内容，包括顶部社区精选卡。新增 AI 可用性基准测试 Skill 与基础体验旅程分析 Skill 两个工具入口。
- 修正作品集封面策略：不再复用不相干页面截图。已采集的 12 个页面使用一一对应的真实预览；未采集页面显示明确的 Archive entry 档案封面，不再假装为页面截图。
...
- “算子开发编程基础”改为章节级放射图：中心为本节主题，第一圈为算子建模/数据排布/Scalar 访存/流水优化四个知识簇，第二圈展示 8 个具体概念；中心与簇、簇与概念均有曲线连接，并保留必要的跨簇依赖。
