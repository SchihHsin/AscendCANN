...
- Qwen3 路径的最后一组课程由“巩固与延展”扩展为“巩固、优化与认证”，从 12 节增至 17 节：增加本地实验工程组织、RMSNorm 小算子到 `torch_npu.npu_rms_norm` 的替换、相同输入下热身 + 三次计时验证、RoPE / 图模式扩展与实验成果认证收口；资源直接关联 `03_qwen3_npu_inference_fused_op.ipynb`、Learning Hub 工程与认证中心。
