...
### 2026-08-20（复杂编码表现与昇腾知识获取）

- 在“运行机制差异”之后新增“机制差异意味着什么”页：以任务 → 规则 / Skill 约束 → 知识库 / MCP 获取昇腾事实 → 验证回填 → 候选交付的结构化链路，解释社区可补强的位置。
- 明确 Skill 的作用是提供版本边界、步骤、分支与停止条件；知识库提供稳定、版本化、可引用的昇腾文档事实；MCP 提供仓库、CI、NPU、日志等实时事实和受控动作；验证把结果转成下一轮可读证据。
- 页面同时标出边界：上述机制可降低无依据猜测和错误恢复成本，但不能单独解释 Claude Opus 更强，也不等同于改变模型本体的训练、推理与编码能力；需要同任务集、同 Host 实测。
