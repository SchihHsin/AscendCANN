...
Banner 只能选择一种模型：卡片式 Hero（Variant A）或整页 body 渐变（Variant B）。选择 Variant B 时，`.hero` 必须是透明结构层，不能再有渐变背景、装饰条、border、shadow 或卡片外壳；选择 Variant A 时，body 必须保持纯背景，不能再启用整页暖色斜向渐变和 `body::before` 白条。
...
### 7.6 Roadmap / 流程图谱（方案 C）
- 适用：学习路径、依赖路径、任务 DAG、能力路线图。默认是右侧 canvas 内的流程图谱，不是竖向卡片列表。
- 画布：900–960px 宽；阶段用淡色底带分区；节点用白底 soft card，最小高度约 108px，内部 padding 18–20px。
- 空间：同阶段节点 gap 22–28px；同阶段上下层 `cgap >= 80px`；阶段间 `inter >= 96px`，为贝塞尔连线留出明确通道。
- 连线：inline SVG，source bottom center → target top center，纵向贝塞尔，stroke 1.6–1.8；阶段主色走线，次要依赖 dashed。
- 层级：阶段容器和节点之间只能有一层视觉 owner；不用 border 外框、左侧强调描边、顶部色条或 inset edge shadow。阶段归属用阶段淡色底、phase label、tag 色和连线颜色表达。
...
- ❌ **不要用边缘高亮描边**（`border-left/top`、顶部色条、inset edge shadow）做卡片 / callout / 图谱节点 — 用背景色、标题、胶囊和数值可视化区分
- ❌ **不要把所有信息都卡片化** — 次要元信息、说明文字、公式解释优先用正文 / `dl` / 轻分隔线
- ❌ **不要把“去卡片化”理解成所有内容纯文字** — 总体结论、摘要洞察、KPI summary 是内容对象，应保留 soft background card
- ❌ **不要做边框套娃** — 避免 `section > card > table/list/callout` 三层边框；保留主容器或数据表其中一层即可
- ❌ **不要给 tabs / TOC 加外层描边壳** — 导航是控制层，默认无背景、无边框；只给 active/hover tab 轻量底色
- ❌ **不要把方法说明 section 做成卡片** — 方法、口径、解释文字是结构信息；保持 plain section，让表格或关键数据承担视觉层
- ❌ **不要用死板后台表格** — 避免大面积灰表头 + 每行整条横线；用轻量 label、行间留白、淡色 tint、score pill / bar 承担层级
- ❌ **不要为了区分楼层加回卡片壳** — 用 32–64px 间距、标题组、1px 顶部 hairline / 渐变线建立 section rhythm
