...
def clean_markdown_file(filepath: str, backup: bool = True) -> bool:
    """
    清理单个 Markdown 文件
...
def clean_markdown_directory(dirpath: str, backup: bool = True, recursive: bool = True) -> tuple:
    """
    ...
    for filepath in sorted(files):
        try:
            ...
            result = clean_markdown_file(filepath, backup=backup)
...
示例:
  ...
  python clean_markdown.py --dir "docs/" --no-recursive
    ...
    if args.file:
        # 清理单个文件
        print(f"\n处理文件: {args.file}")
        success = clean_markdown_file(args.file, backup=backup)
        if success:
            print("\n清理完成!")
        else:
            print("\n清理失败!")
