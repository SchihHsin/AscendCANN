...
// Compute
xLocal = inQueueX.DeQue<T>();     // 阻塞等待MTE2完成
Compute(xLocal);
outQueueY.EnQue(yLocal);

// CopyOut
yLocal = outQueueY.DeQue<T>();    // 阻塞等待Vector完成
DataCopy(yGm, yLocal, size);
```
...
**repeatTime 为 uint8_t 时，最大值 255**

```cpp
// Host 侧限制 R_max
constexpr uint32_t MAX_REPEAT_TIMES = 255;
R_max = std::min(R_max, MAX_REPEAT_TIMES);
```

**详细用法**：[api-vector.md](api-vector.md)
...
**详细用法**：[api-reduce.md](api-reduce.md)
...
5. 检查 repeatTime 是否溢出 → api-vector.md
```
...
| Vector 计算 | [api-vector.md](api-vector.md) | repeatTime 限制、分批处理 |
