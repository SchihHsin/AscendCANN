...
-   核间同步
    ...
    上文描述的都是核内同步的情况。特别的，当算子使用多核同步时（多核同步概念可参考[多核同步](IBSet.md)），逻辑核数BlockDim必须保证不大于实际运行该算子的AI处理器核数，否则框架插入同步会出现异常，导致Kernel“卡死”现象。
...
**图 1**  多核并行处理示意图
![](figures/多核并行处理示意图-40.png "多核并行处理示意图-40")
