...
**表 1** Ascend C  MatMul+LeakyRelu算子设计规格
...
2.  进行MatMul内部的计算，计算公式和计算示意图如下：
    ...
    **图 1**  Matmul矩阵乘示意图
    ![](figures/Matmul矩阵乘示意图-37.png "Matmul矩阵乘示意图-37")
...
4.  进行Vector矢量计算，该样例中进行LeakyReLU计算。
    ...
    ![](figures/zh-cn_formulaimage_0000002534424873.png)

    ![](figures/zh-cn_image_0000002534424865.png)
...
>        ```
>    -   若算子工程不是[自定义算子工程](概述-3.md)，也不是带有[HAVE\_WORKSPACE](基于样例工程完成Kernel直调.md#table481718169817)编译宏的Kernel直调算子工程，kernel侧需要在Matmul初始化前，通过[SetSysWorkSpace](SetSysWorkSpace.md)设置系统workspace。
>        ```
