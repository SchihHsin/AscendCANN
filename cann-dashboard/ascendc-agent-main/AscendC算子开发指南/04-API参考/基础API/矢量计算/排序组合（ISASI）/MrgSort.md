...
-   数据类型为float，每个结构占据8Bytes。
    ...
    ![](figures/zh-cn_image_0000002534414029.png)

-   数据类型为half，每个结构也占据8Bytes，中间有2Bytes保留。

    ![](figures/zh-cn_image_0000002502574218.png)
...
| 参数名 | 描述 |
| --- | --- |
| T | Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持的数据类型为：half/float
...
- 7：前三条队列有效
- 15：四条队列全部有效 |
| repeatTimes | 迭代次数，每一次源操作数和目的操作数跳过四个队列总长度。取值范围：repeatTimes∈[1,255]。
