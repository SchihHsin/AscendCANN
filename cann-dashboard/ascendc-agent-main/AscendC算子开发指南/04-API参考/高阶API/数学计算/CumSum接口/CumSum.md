...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| dstTensor | 输出 | 目的操作数。按first轴或last轴处理，输入元素的累加和。
类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。 |
| lastRowTensor | 输出 | 目的操作数。模板参数config中的outputLastRow参数取值为true时，输出的最后一行数据。
类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。 |
| srcTensor | 输入 | 源操作数。
类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。 |
...
| cumSumInfo | 输入 | srcTensor的shape信息。CumSumInfo类型，具体定义如下：
struct CumSumInfo
{
