...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| basicM | 输入 | 用于设置参数basicM。
与TCubeTiling结构体中的baseM参数含义相同，Matmul计算时base块M轴长度，以元素为单位。 |
| basicN | 输入 | 用于设置参数basicN。
与TCubeTiling结构体中的baseN参数含义相同，Matmul计算时base块N轴长度，以元素为单位。 |
| basicK | 输入 | 用于设置参数basicK。
与TCubeTiling结构体中的baseK参数含义相同，Matmul计算时base块K轴长度，以元素为单位。 |
| singleCoreM | 输入 | 用于设置参数singleCoreM。
单核内M轴shape大小，以元素为单位。 |
| singleCoreN | 输入 | 用于设置参数singleCoreN。
单核内N轴shape大小，以元素为单位。 |
| singleCoreK | 输入 | 用于设置参数singleCoreK。
单核内K轴shape大小，以元素为单位。 |
| stepM | 输入 | 用于设置参数stepM。
左矩阵在A1中缓存的bufferM方向上baseM的倍数。 |
| stepN | 输入 | 用于设置参数stepN。
