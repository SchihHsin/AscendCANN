...
-   update为false：
    ...
    ![](figures/zh-cn_formulaimage_0000002502734482.png)

-   update为true：

    ![](figures/zh-cn_formulaimage_0000002534494523.png)
...
| 参数名 | 描述 |
| --- | --- |
| T | 输入srcTensor及输出dstTensor、expMaxTensor操作数的数据类型。
Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持的数据类型为：half。
Atlas A2 训练系列产品/Atlas A2 推理系列产品，支持的数据类型为：half。 |
| U | 输入inMeanTensor、inExpSumTensor、inMaxTensor及输出meanTensor、expSumTensor、maxTensor操作数的数据类型。
