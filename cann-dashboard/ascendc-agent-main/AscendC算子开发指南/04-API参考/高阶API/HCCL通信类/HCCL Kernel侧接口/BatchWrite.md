...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| batchWriteInfo | 输入 | 通信任务信息的Global Memory地址。一组通信数据的相关信息必须按指定的格式保存，在执行通信任务时，可以同时指定多组通信任务信息，执行通信任务时批量发送数据。
...
| itemNum | 输入 | 批量任务的个数。该参数取值必须与batchWriteInfo中通信任务信息的组数一致。
对于Atlas A3 训练系列产品/Atlas A3 推理系列产品，该参数取值不能大于等于2048。 |
| queueID | 输入 | 指定当前通信所在的队列ID，默认值为0。
