...
| --- | --- | --- |
| srcShape | 输入 |
- 输出归一化结果、均值和方差的LayerNorm接口：输入数据inputX的shape信息{B, S, storageHLength, originHLength}，包括当前输入的inputX的shape信息，以及地址对齐前（如存在H轴补齐操作）的原有shape信息 。
...
| maxValue | 输出 | 输出LayerNorm接口所需的tiling信息（最大临时空间大小）。
...
| minValue | 输出 | 输出LayerNorm接口所需的tiling信息（最小临时空间大小）。
