...
| dataType | 输入 | 输入的数据类型，ge::DataType类型，当前只支持half/float/uint16_t/int16_t数据类型的输入。 |
| config | 输入 | 数据格式转换的场景，参数取值与TransData接口的config参数保持一致。当前支持的转换场景有：NCDHW -> NDC1HWC0、NDC1HWC0 -> NCDHW、NCDHW -> FRACTAL_Z_3D、FRACTAL_Z_3D -> NCDHW。TransDataConfig类型，具体定义如下。
