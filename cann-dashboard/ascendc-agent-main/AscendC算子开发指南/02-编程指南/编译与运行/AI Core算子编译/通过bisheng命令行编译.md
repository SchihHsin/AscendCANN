...
以下是一个使用毕昇编译器编译静态Shape的add\_custom算子入门示例。该示例展示了如何编写源文件add\_custom.asc以及具体的编译命令。通过这个示例，您可以了解如何使用毕昇编译器进行算子编译。
...
4.  采用如下的编译命令进行编译。
    ...
    -   -o demo：指定输出文件名为demo。
    -   --npu-arch=dav-2201：指定NPU的架构版本为dav-2201。dav-后为NPU架构版本号，各产品型号对应的架构版本号请通过[对应关系表](SIMD-BuiltIn关键字和API.md#table65291052154114)进行查询。
