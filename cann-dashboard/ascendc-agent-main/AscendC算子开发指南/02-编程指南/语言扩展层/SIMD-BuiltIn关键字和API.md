...
-   **\_\_NPU\_ARCH\_\_**
    ...
    \_\_NPU\_ARCH\_\_是Device侧AI Core代码中的预处理宏，用于标识AI处理器的架构版本。该宏由四位数字组成，其中前三位数字用于标识AI Core的IP核\(Intellectual Property Core\)类型，第四位数字标识该AI Core同一个IP核的配置版本。通过该宏，开发者可以针对不同AI处理器，差异化进行代码适配和优化。昇腾AI处理器型号和\_\_NPU\_ARCH\_\_对应关系如下表所示：

    **表 1** 昇腾AI处理器型号和\_\_NPU\_ARCH\_\_的对应关系
...
- AscendC::AIC
- AscendC::AIV | 常量值由框架自动设置，AIC核下，配置为AscendC::AIC，AIV核下，配置为AscendC::AIV。
