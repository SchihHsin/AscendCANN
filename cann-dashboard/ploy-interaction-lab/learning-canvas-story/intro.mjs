...
.story-deck .intro-labels{position:relative;z-index:2;align-self:flex-end;display:flex;justify-content:flex-end;gap:.5vw;flex-wrap:wrap;max-width:100%;margin-top:-5.5vh;margin-right:1vw;pointer-events:none}
.story-deck .intro-labels span{font-size:var(--fs-sm);padding:.45vh .75vw;background:rgba(255,255,255,.84);border:1px solid rgba(255,255,255,.92);border-radius:8px;color:var(--ink);box-shadow:0 5px 14px -9px rgba(30,40,70,.5);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}
...
['任务：持续表达目标','改动：Diff 可审阅','结果：在同一现场接续'],
