...
只输出JSON，不加任何其他内容。`,
  ...
  function ldOpenPathNode(index) {
    ...
    const practice = practiceSteps.length ? `<section id="ld-practice-section"><h2>动手练习</h2><div class="ld-practice-steps">${practiceSteps.map((step, stepIndex) => `<article class="ld-practice-step" data-practice-index="${stepIndex}"><button class="ld-practice-step-trigger" type="button" onclick="ldTogglePracticeStep(${stepIndex},this)" aria-expanded="false"><span>${stepIndex + 1}</span><div><strong>${step.title}</strong><small>${step.desc}</small></div><i data-lucide="chevron-down" aria-hidden="true"></i></button><div class="ld-practice-step-detail" hidden><div class="ld-practice-code-head"><div class="ld-practice-code-title"><strong>本步代码</strong><small>Python · 第 ${stepIndex + 1} 步</small></div><small class="ld-practice-expected">运行后预期：${step.expected}</small></div><div class="ld-code-example"><div class="ld-code-toolbar ld-practice-code-toolbar"><div><button type="button" onclick="event.stopPropagation();ldCopyPracticeCode(this,${stepIndex})"><i data-lucide="copy" aria-hidden="true"></i>复制</button><button class="ld-code-run" type="button" onclick="event.stopPropagation();ldOpenLabStep(${stepIndex})"><i data-lucide="play" aria-hidden="true"></i>在 HiDevLab 运行</button></div></div><pre>${escHtml(step.code)}</pre></div></div></article>`).join('')}</div></section>` : '';
  ...
  function ldRenderVideoLinkedContent(segmentIndex) {
    ...
    const practiceCodeBlock = step?.code ? `<div class="ld-code-example"><div class="ld-code-toolbar ld-practice-code-toolbar"><div><button type="button" onclick="ldCopyPracticeCode(this,${segment.practice})" title="复制代码"><i data-lucide="copy" aria-hidden="true"></i>复制</button></div></div><pre>${escHtml(step.code)}</pre></div>` : '';
    const practice = step ? `<section class="ld-video-linked-practice"><h3>对应动手练习</h3><div class="ld-video-practice-step"><span>${segment.practice + 1}</span><div><strong>${step.title}</strong><small>${step.desc}</small></div></div>${practiceCodeBlock ? `<div class="ld-video-practice-code"><div class="ld-practice-code-head"><div class="ld-practice-code-title"><strong>本步代码</strong><small>Python · 第 ${segment.practice + 1} 步</small></div><small class="ld-practice-expected">运行后预期：${step.expected}</small></div>${practiceCodeBlock}</div>` : ''}<button class="ld-video-practice-run" type="button" onclick="ldOpenLabStep(${segment.practice})"><i data-lucide="play" aria-hidden="true"></i>在 HiDevLab 运行这一步</button></section>` : '';
