...
只输出JSON，不加任何其他内容。`,
  ...
  function ldRefreshStudyTools(node, knowledge) {
    ...
    const clusters = (graph.clusters || []).map(item => `<div class="ld-kv-cluster ${item.group}" style="top:${item.y}px"><strong>${item.title}</strong><small>${item.subtitle}</small></div>`).join('');
  ...
  function ldBuildKnowledgeGraph(node, knowledge) {
    ...
    return { clusters:[
      { group:'model', title:'算子建模', subtitle:'定义计算契约', y:0 },
      { group:'layout', title:'数据排布', subtitle:'决定访问与计算适配', y:88 },
      { group:'scalar', title:'Scalar 访存', subtitle:'控制与数据读写', y:176 },
      { group:'flow', title:'流水优化', subtitle:'隐藏搬运等待', y:264 }
    ], nodes:[
      { id:'operator', label:'算子', title:'算子基本概念', desc:'算子是模型中的计算单元；先定义输入、输出、数据类型与计算规则。', group:'model', x:0, y:38, source:docs.operator },
      { id:'io', label:'输入 / 输出', title:'逻辑输入与输出', desc:'输入、输出及其 shape、数据类型共同描述算子的计算契约。', group:'model', x:99, y:38, source:docs.operator },
      { id:'logic', label:'计算逻辑', title:'计算规则', desc:'将数学计算拆成可在 AI Core 上执行的数据搬运与计算步骤。', group:'model', x:198, y:38, source:docs.operator },
      { id:'tensor', label:'Tensor', title:'Tensor 数据排布', desc:'数据排布定义多维 Tensor 在内存中的存储方式。', group:'layout', x:0, y:126, source:docs.layout },
      { id:'nd', label:'ND / NCHW', title:'通用数据格式', desc:'ND、NCHW、NHWC 为维度赋予业务语义，通道位置会影响访问特性。', group:'layout', x:99, y:126, source:docs.layout },
      { id:'nz', label:'FRACTAL_NZ', title:'分形数据格式', desc:'NZ 等格式通过分块布局适配 Cube 计算单元，服务矩阵计算效率。', group:'layout', x:198, y:126, source:docs.layout },
      { id:'gmub', label:'GM ↔ UB', title:'Scalar 可访问存储', desc:'Scalar 仅支持读写 Global Memory 和 Unified Buffer。', group:'scalar', x:0, y:214, source:docs.scalar },
      { id:'scalar', label:'Get / Set', title:'Scalar 读写', desc:'Scalar 通过 GetValue、SetValue 操作数据，属于 PIPE_S 流水操作。', group:'scalar', x:99, y:214, source:docs.scalar },
      { id:'sync', label:'DataCache 同步', title:'缓存一致性', desc:'GM 标量访问经过 DataCache；多核共享数据变化时需要保证一致性。', group:'scalar', x:198, y:214, source:docs.scalar },
      { id:'pipeline', label:'Copy + Compute', title:'搬运与计算流水', desc:'CopyIn、Compute、CopyOut 组成典型的数据搬运与计算过程。', group:'flow', x:0, y:302, source:docs.buffer },
      { id:'queues', label:'MTE / Vector', title:'独立指令队列', desc:'MTE 搬运队列和 Vector 计算队列相互独立，可并行执行。', group:'flow', x:99, y:302, source:docs.buffer },
      { id:'double', label:'DoubleBuffer', title:'双缓冲优化', desc:'将数据分两块，让一块计算时另一块搬运或回写，以隐藏等待时间。', group:'flow', x:198, y:302, source:docs.buffer }
    ], edges:[
